...
def predict(api_key, model_name, prompt, temperature=0, logprobs=5, n=1, stop=None, max_tokens=1024, echo=False, min_query_interval=None):
	chat = model_name.startswith('gpt-4.1')
	if chat:
	...
	try:
		with urllib.request.urlopen(request) as response:
			...
			if len(outputs) != n:
				print('gpt4.predict ERROR: The length of the `choices` field is not `n`.')
				return None, None
			if outputs[0]['finish_reason'] != 'stop':
				print('gpt4.predict WARNING: GPT4 stopped predicting tokens for a reason other than predicting a STOP token.')
			if logprobs == None:
				if n != 1:
					if chat:
